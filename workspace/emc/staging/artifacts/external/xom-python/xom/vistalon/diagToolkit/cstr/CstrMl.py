...
'''
  Output is percentage change in either H2 or cat flow required to get Ml back on target.  This will appear as incremental output to the destination, a percentage change tag.  
'''
...
def calculate(application, fd, provider, database):
    ...
    if not (grade.quality.isGood()):
        postApplicationMessage(application, QUEUE_ERROR, "%s - unable to read the current grade: %s" %(__name__, str(grade.quality)))
        return textRecommendation, recommendations
    grade=grade.value
    
    log.info("In %s, the grade is: %s" % (__name__, str(grade)))
    
    tagName="[%s]LabData/RLA3/MOONEY-FILTERED-VALUE/filteredValue" % (provider)
    ...
    if not (sp.quality.isGood()):
        postApplicationMessage(application, QUEUE_ERROR, "%s: %s - SP is bad (%s) %s" % (__name__, tagName, str(sp.quality)), log)
        return textRecommendation,recommendations
    sp = sp.getValue()
    ...
    log.trace("ML-CAT gain: %s" % (str(gainCat)))
    log.trace("ML-H2 gain: %s" % (str(gainH2)))
    log.trace("ML-C3 gain: %s" % (str(gainC3)))
    
    postApplicationMessage(application, QUEUE_INFO, "%s will use fb data of gain = %s or %s as appropriate, ff data of gain = %s, PV = %s and SP = %s." % (__name__, str(gainCat), str(gainH2), str(gainC3), str(pv), str(sp)), log)
